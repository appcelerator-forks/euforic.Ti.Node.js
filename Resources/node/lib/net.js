...
Socket.prototype.setEncoding = function(encoding) {
  var StringDecoder = node.require('string_decoder').StringDecoder; // lazy load
...
Socket.prototype.connect = function(options, cb) {
  ...
  if (pipe) {
    ...
    // node.require('dns').lookup(host, function(err, ip, addressType) {
    ...
    // #TINODE_START
      connect(self, host, options.port, 4);
    // #TINODE_END
  }
  return self;
};
...
function listen(self, address, port, addressType, backlog, fd) {
  if (!cluster) cluster = node.require('cluster');
...
Server.prototype.listen = function() {
  ...
  if (arguments.length == 0 || typeof arguments[0] == 'function') {
    ...
    node.require('dns').lookup(arguments[1], function(err, ip, addressType) {
      if (err) {
        self.emit('error', err);
      } else {
